...
public class WaveformView extends View {
    ...
    /** Callback fired when the user scrubs the waveform to a new position. */
    public interface SeekListener {
        /** @param fraction 0.0 – 1.0 representing the scrubbed position */
    ...
    private static final int   COLOR_PLAYED  = 0xFF9A81FF;   // ds_accent — Brand Lavender
    private static final int   COLOR_UNPLAYED = 0xFF3A3548;  // ds_divider — muted violet-dark
    private static final int   COLOR_THUMB   = 0xFFFFFFFF;
    private static final float THUMB_RADIUS_DP = 6f;         // slightly larger for visibility
    ...
    /**
     * Attach a listener that fires as the user drags their finger across the waveform.
     * Pass {@code null} to disable seeking (e.g. during recording).
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN
            ...
            float fraction = Math.max(0f, Math.min(1f, event.getX() / Math.max(1f, getWidth())));
            // Update thumb instantly so the UI responds before the MediaPlayer seek completes
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < bars.size(); i++) {
            ...
            // Near-silent samples (pauses between words) get a visibly thinner bar
            // instead of the same width as real signal — this is the taper WhatsApp
            // shows during quiet stretches, and it was missing entirely here since
            // every bar previously used the same full barW regardless of amplitude.
            float thisBarW = norm < 0.12f ? Math.max(1f, barW * 0.4f) : barW;
            float rightPad = (barW - thisBarW) / 2f;
            float right   = left + barW - rightPad;
            left += rightPad;
            float bottom  = top + barH;

            Paint paint = (i < playedUpTo) ? paintPlayed : paintUnplayed;
            canvas.drawRoundRect(left, top, right, bottom, thisBarW / 2f, thisBarW / 2f, paint);
        }
