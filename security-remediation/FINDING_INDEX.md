...
| S03-L1 | Low | `app/build.gradle:75-77` | TB-9/Theme D | `WORKER_SECRET` still compiled into APK (dup of S08-H1) | **fixed** (S3-01, 2026-08-11) — `app/build.gradle` no longer emits `BuildConfig.WORKER_SECRET` nor reads it from `local.properties`/env (remediated in R1; re-confirmed from source this session). Worker-side acceptance closed jointly with S08-H1 in `6c30267`. | P1 | R1 | **verified-source** 2026-08-11: `grep -rn WORKER_SECRET app/src` finds only SEC-A01 comments, no `BuildConfig.WORKER_SECRET` reference; `buildConfigField` list in `app/build.gradle` carries no WORKER_SECRET | fixed |
