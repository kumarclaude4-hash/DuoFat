 ...
 *   - The XEd25519 spec defines the message as prefixed with a domain-separation
 *     constant when produced by Signal's implementation. This module expects the
 *     caller to apply the same prefix to `message` before calling verifySignature.
 *
 * Usage:
 ...
 *
 * Signal's calculateSignature prepends a 32-byte domain-separation prefix
 * (0xFE repeated 32 times) before performing the Ed25519 sign operation
 * (XEd25519 spec §2 "nonce and message"). We prepend the same prefix here.
 *
 * @param  {Buffer} curve25519PubKey  32-byte Curve25519 public key
 * @param  {Buffer} message           Original message (without prefix)
...
function verifySignature(curve25519PubKey, message, signature) {
  try {
    ...
    // XEd25519 domain prefix: 32 × 0xFE (Signal spec §2).
